...
from criticker_movies.items import CritickerGamesItem
import scrapy
...
class GamesSpider(MoviesSpider):
    ...
    start_urls = [
        "https://games.criticker.com/games"
    ]

    def parse_item(self, response: scrapy.http.response.Response, on_netflix: bool) -> CritickerGamesItem:
        """
        ...
        for i, hi in enumerate(h):
            ...
            if 'aka' in label:
                game_data[label] = response.xpath('//p[@id="{}"]/text()'.format(hi_)).extract_first()
            else:
                game_data[label] = self.extract_more_info(hi)
        game_data['trailer_url'] = response.xpath('//div[@id="fi_trailer"]/iframe/@src').extract_first()
        if game_data['trailer_url'] == 'http://www.youtube.com/watch?v=':
            game_data['trailer_url'] = None
        game_data['rss_feed_url'] = response.xpath('//*[@id="fi_titlerss"]/a/@href').extract_first()
        game_data['avg_percentile'] = response.xpath('//span[@itemprop="ratingValue"]/text()').extract_first()
        game_data['n_ratings'] = response.xpath('//span[@itemprop="reviewCount"]/text()').extract_first()
        return game_data
